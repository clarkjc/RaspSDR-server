...
static bool _cfg_lookup_json_cb(cfg_t* cfg, void* param1, void* param2, jsmntok_t* jt, int seq, int hit, int lvl, int rem, void** rval) {
    ...
    bool rv = true;
    if (lvl_match > 0 && lvl != lvl_match)
        rv = false; // lvl_match > 0 means match lvl
    else if (lvl_match < 0 && lvl == lvl_match)
        rv = false; // lvl_match < 0 means reject lvl
    else if (n != id_match_len || strncmp(s, id_match, n) != 0)
        rv = false;
...
int _cfg_set_int(cfg_t* cfg, const char* name, int val, u4_t flags, int pos) {
    ...
    else if (flags & (CFG_SET | CFG_CHANGE)) {
        if (!jt || jt == CFG_LOOKUP_LVL1) {
            char* int_sval;
...
#define INDENT 4
            ...
            if (flags & FL_PANIC) { panic("jsmn_parse"); }
            else
                return false;
        }
