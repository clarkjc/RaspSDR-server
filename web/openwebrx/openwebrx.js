...
var owrx = {
   ...
   FSET_PB_CHANGE: 4,
   FSET_SCALE_DRAG: 5,
   FSET_SCALE_DRAG_END: 6,
   FSET_SCALE_MOUSE_CLICK: 7,
   FSET_SCALE_TOUCH_DRAG: 8,
...
function scale_canvas_start_drag(evt, isMouse)
{
	...
	if (evt.button == mouse.right && !evt.ctrlKey) {
		right_click_menu(scale_canvas_drag_params.start_x, scale_canvas_drag_params.start_y, 1);
...
function scale_canvas_drag(evt, x, y)
{
   if (scale_canvas_ignore_mouse_event) return;
	...
	if (scale_canvas_drag_params.drag) {
		...
		if (!event_handled)
			demodulator_set_offset_frequency(owrx.FSET_SCALE_DRAG, scale_offset_carfreq_from_px(x));
...
function scale_canvas_end_drag(evt, x, canvas_mouse_up)
{
   ...
   if (scale_canvas_drag_params.mouse_down == true && evt.type == "mouseout") {
	...
	if (scale_canvas_drag_params.mouse_down == true) {
      ...
      if (!event_handled) demodulator_set_offset_frequency(owrx.FSET_SCALE_DRAG_END, scale_offset_carfreq_from_px(x));
   }
...
function canvas_mouseout(evt)
{
   ...
   // must also end dragging when mouse leaves canvas while still down
	if (canvas_dragging) {
      ...
      if (owrx.debug_drag) canvas_log('C-MOUT'+ (ignore? ' SPEC-IGNORE':''));
      if (!ignore)
         canvas_end_drag2();
   }
         ...
         if (owrx.double_touch_start) {
            if (owrx.double_touch_swipe) {
               ...
               if (idx == 0 && evt.touches.length >= 2) {
                  ...
                  demodulator_set_offset_frequency(owrx.FSET_SCALE_TOUCH_DRAG, canvas_get_carfreq_offset(relX, true));
               }
            } else {
               // pinch zoom
   ...
   var no_clear_last_gid = w3_opt(opt, 'no_clear_last_gid', 0);
	...
	if (isArg(mode) && (mode != cur_mode || open_ext == true)) {
		...
		if (!no_set_freq) {
         ...
         demodulator_set_offset_frequency(owrx.FSET_SET_FREQ, freq_car_Hz - center_freq);
      }
	}
}
...
function vfo_update(A_equal_B)
{
	if (owrx.vfo_first) {
      var vfo_default = freq_displayed_kHz_str_with_freq_offset + cur_mode +'z'+ zoom_level;
	   var last_vfo_a = parseInt(initCookie('last_vfo_A', vfo_default));
	   var last_vfo_b = parseInt(initCookie('last_vfo_B', vfo_default));
...
var up_down = {
   // 0 means use special step logic
	...
	qam:  [ 0, -1, -0.1, 0.1, 1, 0 ],
	drm:  [ 0, -1, -0.1, 0.1, 1, 0 ],
	usb:  [ 0, -0.1, -0.1, 0.1, 0.1, 0 ],
	usn:  [ 0, -0.1, -0.1, 0.1, 0.1, 0 ],
	lsb:  [ 0, -0.1, -0.1, 0.1, 0.1, 0 ],
	lsn:  [ 0, -0.1, -0.1, 0.1, 0.1, 0 ],
...
function dx_label_step(dir)
{
   ...
   if (dir == 1) {
      for (i = 0; i < dx.displayed.length; i++) {
         dl = dx.displayed[i];
         if (!dl) continue;
         var f = Math.round(dl.f_label);
         var same_gid = (dl.gid == dx.last_stepped_gid);
         var f_ok = (f > f_pbc_Hz);
	...
	w3_el("id-optbar-wf").innerHTML =
      ...
      w3_col_percent('',
         ...
         w3_divs('/w3-tspace-4 w3-hcenter w3-font-11_25px',
            ...
            w3_inline('',
               w3_button('id-button-spec-peak0 class-button w3-noactive w3-hold|border: 2px solid yellow; padding: 1px 3px|' +
                  'title="toggle peak hold\n#1: off-on-hold\nshift: toggle backwards"', 'P1', 'toggle_or_set_spec_peak', 0),
               w3_button('id-button-spec-peak1 w3-margin-L-4 class-button w3-noactive w3-hold|border: 2px solid magenta; padding: 1px 3px|' +
...
function freq_vfo_cb(el, val, trace, ev)
{
   ...
   w3_innerHTML('id-freq-vfo', "AB"[owrx.vfo_ab]);
	...
	var vfo = parse_freq_pb_mode_zoom(readCookie('last_vfo_'+ "AB"[owrx.vfo_ab]));
	var mode = undefined, zoom = 0;
